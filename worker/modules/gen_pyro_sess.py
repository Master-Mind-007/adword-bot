...
from worker import bot, LOGGER
from worker.helper.ext_utils.bot_utils import new_thread, new_task
from worker.helper.telegram_helper.message_utils import sendMessage, editMessage, deleteMessage, sendFile
...
async def genPyroString(client, message):
    global isStop
    session_dict.clear()
    sess_msg = await sendMessage(message, """<u><i><b>𝐏𝐲𝐫𝐨𝐠𝐫𝐚𝐦 𝐒𝐭𝐫𝐢𝐧𝐠 𝐒𝐞𝐬𝐬𝐢𝐨𝐧 𝐆𝐞𝐧𝐞𝐫𝐚𝐭𝐨𝐫</b></i></u>
...
<i>Send /stop to Stop Process</i>""")
    ...
    await sleep(1.5)
    await editMessage(sess_msg, """<u><i><b>𝐏𝐲𝐫𝐨𝐠𝐫𝐚𝐦 𝐒𝐭𝐫𝐢𝐧𝐠 𝐒𝐞𝐬𝐬𝐢𝐨𝐧 𝐆𝐞𝐧𝐞𝐫𝐚𝐭𝐨𝐫</b></i></u>
...
<i>Send /stop to Stop Process</i>""")
        ...
        async with session_lock:
            if session_dict['CONFIRM_PHN'].lower() in ['y', 'yes']:
                break
    try:
        pyro_client = Client(f"TB-{message.from_user.id}", api_id=api_id, api_hash=api_hash)
...
<i>Send /stop to Stop Process</i>""")
    ...
    try:
        ...
        await sleep(1.5)
        await editMessage(sess_msg, f"""<u><i><b>𝐏𝐲𝐫𝐨𝐠𝐫𝐚𝐦 𝐒𝐭𝐫𝐢𝐧𝐠 𝐒𝐞𝐬𝐬𝐢𝐨𝐧 𝐆𝐞𝐧𝐞𝐫𝐚𝐭𝐨𝐫</b></i></u>
 ...
 <i>Send /stop to Stop Process</i>""")
    ...
    try:
        ...
        await pyro_client.send_message("self", f"<b><u>𝐏𝐲𝐫𝐨𝐠𝐫𝐚𝐦 𝐒𝐞𝐬𝐬𝐢𝐨𝐧 𝐆𝐞𝐧𝐞𝐫𝐚𝐭𝐞𝐝 :</u></b>\n\n<code>{session_string}</code>\n\n<b>Via ADword</b>", disable_web_page_preview=True)
        await pyro_client.disconnect()
        await editMessage(sess_msg, "⌬ <u><i><b>𝐏𝐲𝐫𝐨𝐠𝐫𝐚𝐦 𝐒𝐭𝐫𝐢𝐧𝐠 𝐒𝐞𝐬𝐬𝐢𝐨𝐧 𝐆𝐞𝐧𝐞𝐫𝐚𝐭𝐨𝐫</b></i></u> \n\n➲ <b>String Session is Successfully Generated ( Saved Messages ).</b>")
    except Exception as e:
        return await editMessage(sess_msg ,f"<b>𝐄𝐱𝐩𝐨𝐫𝐭 𝐒𝐞𝐬𝐬𝐢𝐨𝐧 𝐄𝐫𝐫𝐨𝐫:</b> {str(e)}")
    try:
        await aioremove(f'ADw_{message.from_user.id}.session')
        await aioremove(f'ADw-{message.from_user.id}.session-journal')
    except: pass
...
async def set_details(_, message, newkey):
    ...
    if value.lower() == '/stop':
        isStop = True
        return await editMessage(session_dict['message'], '<b>𝐏𝐫𝐨𝐜𝐞𝐬𝐬 𝐒𝐭𝐨𝐩𝐩𝐞𝐝</b>')
...
async def invoke(client, message, key):
    ...
    while session_dict[user_id]:
        ...
        if time() - start_time > 120:
            session_dict[user_id] = False
            await editMessage(message, "<b>𝐏𝐫𝐨𝐜𝐞𝐬𝐬 𝐒𝐭𝐨𝐩𝐩𝐞𝐝</b>")
            isStop = True
